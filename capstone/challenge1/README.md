# Capstone ハンズオン 課題1 - サーバーレスの 生成 AI Web アプリケーションの構築
...
3. サービス検索で「**`lambda`**」入力して Lambda のページを開きます
4. 「**関数の作成**」をクリックします
5. 以下の設定で関数を作成します：
...
4. 「**許可**」タブから「**許可を追加**」→「**ポリシーをアタッチ**」をクリックします
5. 検索ボックスに `my-Lambda-Bedrock-role` と入力します
6. 「**my-Lambda-Bedrock-role**」にチェックを入れ、「**許可を追加**」をクリックします
    - もし my-Lambda-Bedrock-role が見つからない場合は、インストラクターまでご連絡ください
...
* Lambda 関数のコードを変更して、converse API で Amazon Nova Lite v1 へ問い合わせを行う処理を完成して下さい。
    - モデル ID は `us.amazon.nova-lite-v1:0` を使用して下さい。
    - コードを変更後は、「**Deploy**」をクリックしてください。 

* コード修正後、ステップ 8 の動作確認を行い基盤モデルを正しく呼び出せていることを確認してください。
---
