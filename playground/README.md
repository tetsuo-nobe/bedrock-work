...
1. ページ右上の [**比較モード**] のトグルを有効化します。
    - モデルの結果表示エリアが左右２つに分割されます。
1. [**モデルを選択**] をクリックします。
1. [**1. カテゴリ**] で [**Amazon**] をクリックします。
1. [**2. モデル**] で [**Nova Pro 1.0**] をクリックします。
1. [**3. 推論**] で [**推論プロファイル**] - [**US Nova Pro**] が選択されていることを確認します。
1. [**適用**] をクリックします。
1. [**設定**] の　右側に表示されている [**<**] をクリックして [**設定**] の表示を閉じます。
1. ページ下部の入力エリアに下記を入力します。

    - ```
      AI の定義について、わかりやすく説明して下さい。
      ```

1. [実行] をクリックします。
1. Nova Lite と Nove Pro の出力結果を確認します。
    - この機能は、２つの基盤モデルの出力を比較するのに適しています。
1. ページ右上の [**比較モード**] のトグルを無効化します。

## プロンプトの最適化を行ってみましょう

* この機能は英語のみ対応していますが、日本語から最適化して推論結果を日本語で表示する形で試してみましょう

1. ページ下部の入力エリアに下記を入力します。

    - ```
      AI の定義について、わかりやすく説明して下さい。結果は日本語で表示して下さい。
      ```

1. 入力エリアの下側にある「魔法の杖」のアイコン（プロンプトを最適化のアイコン）をクリックします。
    - 最適化されたプロンプトが表示されるまで少し待ちます。
1. 「最適化されたプロンプト」の内容を確認し、「最適化されたプロンプトを使用」をクリックします。
    - 入力エリアに最適化されたプロンプトが適用されます。
1. [実行] をクリックして結果を確認します。

### お疲れさまでした！Amazon Bedrock の Playground で基盤モデルを選択して使用や比較ができることを確認しました。
