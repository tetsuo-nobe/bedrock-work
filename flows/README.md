...
1. 推論の設定で長さを `4096` に変更します。

1. キャンバスで、Prompts ノードの  Input の dest と count の左横の丸を、Flow Inputノードの丸にドラッグアンドドロップでつなげます。

1. キャンバスで、Flow Inputノードを選択します。

1. 左側で、出力の document のタイプを Object に変更します。
...
1. バケット名に以下を入力。(**99 のところは自分の2桁の番号に置き換えます。**)
...
1. 左側のフロービルダーの ノードタブから、**S3 ストレージ**　をキャンバスにドラッグアンドドロップで、Flow output ノードの下あたりに配置します。
